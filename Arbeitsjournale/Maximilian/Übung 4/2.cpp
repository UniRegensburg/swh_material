...
//frequencies for each note. Copied from pitches.h library
#define NOTE_B0  31
#define NOTE_C1  33
#define NOTE_CS1 35
#define NOTE_D1  37
...
// note durations: 8 = 8th note, 4 = quarter note, etc.
//negative values represent dotted notes
int durations[] = {
-4, -4, -4, -4,
8, -4, 8, 8, 8, 8,
8, 8, 8, 8, 8, 8, 8, 8,
...
// length of a whole note in ms (can be adapted by changing the tempo)
...
void loop() {
  for (int thisNote = 0; thisNote < songLength; thisNote++) {
    ...
    if (noteDur > 0) { //regular note
      noteDuration = (wholenote) / noteDur;
    } else if (noteDur < 0) { //dotted note (neg value)
    ...
    noTone(speaker); //stop current note
  }

delay(3000); //3s delay before new cycle begins
